...
$retrieve.EntityLogicalName = 'gr_equipment'
$retrieve.LogicalName = 'gr_currenthourmeterrecordeddate'
$retrieve.RetrieveAsIfPublished = $true
try {
    $service.Execute($retrieve) | Out-Null
    Write-Output 'gr_equipment.gr_currenthourmeterrecordeddate already exists'
    exit 0
} catch {
...
$attribute.SchemaName = 'gr_CurrentHourMeterRecordedDate'
$attribute.DisplayName = [Microsoft.Xrm.Sdk.Label]::new('Current Hour Meter Recorded Date', 1033)
...
$publish.ParameterXml = '<importexportxml><entities><entity>gr_equipment</entity></entities></importexportxml>'
$service.Execute($publish) | Out-Null
Write-Output 'Created and published gr_equipment.gr_currenthourmeterrecordeddate'
