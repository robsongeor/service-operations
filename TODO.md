...
- [ ] Define and implement authoritative customer/site PO-request recipients so Chargeable Invoice Review can default the correct recipients without manual entry. Keep email opening as an explicit editable-draft action and decide on an approved attachment-capable integration before replacing the current manual approval-PDF attachment step.
