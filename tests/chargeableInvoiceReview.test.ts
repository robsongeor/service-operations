...
test('Chargeable Invoice route uses shared page primitives and delegates intake and queue workflows', async () => {
    ...
    assert.match(screen, /loadGlobalOperationalData:\s*false/)
    assert.match(screen, /scopedData:\s*EMPTY_INVOICE_JOB_SCOPE/)
    assert.doesNotMatch(screen, /prepareJobEditorReferenceData\(\)/)
    assert.match(screen, /searchEquipmentForEditor\(query, \{\}, controller\.signal\)/)
    assert.match(screen, /equipmentLookupPending/)
    assert.match(screen, /equipmentLookupError/)
    assert.match(screen, /setEquipmentLookupAttempt/)
    assert.match(screen, /mechanicsLoading=\{jobs\.mechanicsLoading\}/)
    assert.match(screen, /onSearchCustomers=\{jobs\.searchCustomersForEditor\}/)
    ...
    assert.match(queue, /Invoice register/)
    assert.match(queue, /<span>Search invoice reviews<\/span>/)
    assert.doesNotMatch(queue, /className="chargeable-preview-intro"><div><h2>/)
    assert.match(workspaceStyles, /\.chargeable-table th[^}]*text-transform: uppercase/)
    assert.match(workspaceStyles, /\.chargeable-queue-tabs button\.active[^}]*background: #edf7f3/)
