...
export default function ChargeableInvoiceQueue() {
    ...
    return <>
        <section className="chargeable-queue-summary">
            <div><span>Invoice register</span><h2>{queue === 'all' ? 'Active chargeable invoice reviews' : queueLabel(queue)}</h2></div>
            <strong><small>Current view</small>{filtered.length} review{filtered.length === 1 ? '' : 's'}</strong>
        </section>
        <section className="chargeable-queue-controls" aria-label="Review queue filters">
            <label className="chargeable-search"><span>Search invoice reviews</span><input type="search" value={search} onChange={(event) => setSearch(event.currentTarget.value)} placeholder="Search invoice, Job, customer or fleet" /></label>
            <div className="chargeable-queue-filter-actions">
                ...
                <button type="button" className="chargeable-secondary chargeable-queue-refresh" disabled={reviews.isLoading} onClick={() => void reviews.refresh()}>Refresh</button>
            </div>
        </section>
        ...
        {reviews.selectedId && <ChargeableInvoiceWorkspace workspace={reviews.workspace} loading={reviews.isLoadingWorkspace} saving={reviews.isSaving} error={reviews.workspaceError} onStart={reviews.startReview} onSaveWaiting={reviews.saveWaiting} onSaveRequirements={reviews.saveRequirements} onPreparePhotoRequest={reviews.preparePhotoRequest} onPreparePoRequest={reviews.preparePoRequest} onUploadPhotos={reviews.uploadPhotos} onDeletePhotos={reviews.deletePhotos} onMarkReady={reviews.markReady} onReturnToInProgress={reviews.returnToInProgress} onMarkDoNotProcess={reviews.markDoNotProcess} onDelete={reviews.deleteReview} onAddCorrection={reviews.addCorrection} onReplaceCorrection={reviews.replaceCorrection} onSupersedeCorrection={reviews.supersedeCorrection} onLoadDocument={reviews.loadDocument} onDownload={reviews.downloadDocument} onLoadQuoteLines={reviews.loadQuoteLines} onGenerateApprovalPdf={reviews.generateApprovalPdf} onClose={closeWorkspace} />}
    </>
}
