...
function markerIcon(equipmentCount: number, selected: boolean) {
    return L.divIcon({
        className: `equipment-map-marker-host${selected ? ' selected' : ''}`,
        html: `<span class="equipment-map-marker">${equipmentCount}</span>`,
        iconSize: [34, 34],
        iconAnchor: [17, 17],
    })
}
...
export default function EquipmentLocationMap({ sites, selectedSiteId, onSelectSite }: Props) {
        ...
        try {
            ...
            markersRef.current = mappedSites.map((site) => {
                ...
                const marker = L.marker([coordinate.latitude, coordinate.longitude], {
                    icon: markerIcon(site.equipment.length, false),
                    keyboard: true,
                    title: `${site.customerName} · ${site.siteName} · ${site.equipment.length} Equipment`,
                    riseOnHover: true,
    ...
    return <div className="equipment-map-canvas-shell">
        <div ref={containerRef} className="equipment-map-canvas" aria-label="Map of Equipment Site locations" />
