...
export async function fetchSites(accessToken: string): Promise<Site[]> {
    const result = await fetch(
        `${DATAVERSE_URL}/api/data/v9.2/gr_sites?$select=gr_siteid,gr_name,gr_address,gr_defaultmaintenanceprofile,gr_geocodelatitude,gr_geocodelongitude,gr_geocodesourceaddress,gr_geocodeformattedaddress,gr_geocoderesolvedon&$expand=gr_Customer($select=gr_customerid,gr_name)`,
        {
            headers: {
