...
export default function QualificationManager({ mechanic, qualifications, qualificationTypes, busy, onCreate, onUpdate, onDeactivate }: Props) {
    ...
    return <section className="mechanic-qualifications" onKeyDown={(event) => { if (editing !== undefined && event.key === 'Enter' && !event.defaultPrevented && event.target instanceof HTMLElement && event.target.tagName !== 'TEXTAREA') { event.preventDefault(); void save() } }}>
        <div className="mechanic-qualifications-heading"><div><strong>Qualifications</strong><span>Manage reusable qualifications and certification periods.</span></div><button type="button" onClick={() => openForm(null)}>Add Qualification</button></div>
        {qualifications.length === 0 ? <p className="mechanic-qualification-empty">No qualifications have been added to this technician.</p> : <div className="mechanic-qualification-list">{qualifications.map((qualification) => {
